...
- `2`: 操作、stdout/stderrまたは画面、判断、失敗した操作、エラー主要行、修正差分、再実行結果が時系列で追跡できる。実際の失敗があるのに要約だけなら2点にしない。

### Expertise

- `0`: 手順の羅列で、仕組み、設定理由、データフロー、適用範囲を説明していない。
- `1`: 理由や制約の一部を説明するが、解決課題、候補比較、選定・不採用理由、適用範囲のいずれかが不足する。仕組み図だけの場合も1点以下。
- `2`: 仕組みとデータフローに加え、解決課題、採用構成、最低1つの不採用案、選定・不採用理由、適用・非適用条件を一次情報または実測と結びつけて説明する。
...
- `2`: 実験、比較、失敗、追加検証のいずれかから、公式資料だけでは得られない再利用可能な洞察を導く。失敗を根拠にする場合、実エラー、切り分け、修正差分、再実行Evidenceまで追跡できる。
...
- `2`: 読者の問いに答え、具体的な行動、技術選定基準、実行可能な失敗回避策、アクセス可能な再現成果物を提供する。エラーと修正commandがない一般的TIPSだけでは2点にしない。
