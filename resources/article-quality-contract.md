...
- `## 中核技術の役割`: 初学者が主題を理解できるよう、`中核技術の定義 / 解決する課題 / 今回なぜ必要か`を対応させる。定義を一次情報へ結び、今回のResearch Questionと検証で必要な理由まで説明する。一般的な製品紹介だけで代替しない。
...
- [ ] 中核技術の定義、解決する課題、今回なぜ必要かが対応し、定義が一次情報へ結びついている
- [ ] 仕組みとデータフローが説明されている
...
`中核技術の役割`、`技術選定理由`、失敗記録の固定ラベルが存在しても、定義・解決課題・今回の必要性、採用理由、エラー、修正、再実行Evidenceが空または相互に無関係ならFAILとする。実障害でエラーメッセージEvidenceが欠ける場合はReader-visible GateをFAILにし、`QUALITY_READY`にしない。これらのGateから品質項目の点数上限を機械的に決めない。
