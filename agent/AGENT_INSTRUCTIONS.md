...
- 初学者が記事の主題を理解するために必要な中核技術について、`中核技術の定義 / 解決する課題 / 今回なぜ必要か`を記事内で対応させる。定義は一次情報へ結び、一般的な製品紹介だけで終えず、今回のResearch Questionと検証で必要な理由を説明する。
- 仕組み図の存在だけでExpertiseを満たしたと扱わない。解決したい課題、今回採用した構成、その構成を優先して試すと決めた理由、適用条件・制約を記事内で対応させる。採用理由をResearch Question、実験制約、実測または一次情報へ結ぶ。比較候補や不採用案は、実際に比較した場合だけ任意で記載する。
- 実際に失敗が発生した記事では、失敗を要約だけで済ませない。エラーメッセージを最重要Evidenceとし、失敗した操作、エラー全文または主要行、原因、切り分け、効果がなかった方法、修正内容、再実行、再実行結果をReader-visible Evidenceとして残す。GUI障害は該当スクリーンショットまたは公開ログを許可する。
...
- 仮説と結果が対応している
- 失敗と修正が隠されていない
- 技術選定が課題、採用構成、採用理由、適用条件・制約と対応している
- 中核技術の定義、解決する課題、今回必要な理由が対応している
