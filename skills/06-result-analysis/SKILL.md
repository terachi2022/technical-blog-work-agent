...
1. 証拠の欠落を確認する。
2. 各仮説と対応する実験結果を紐づける。
3. 仮説ID、反証条件、実測、判定、Evidence IDを同じ表へ整理する。
...
10. 追加実験が必要な論点を列挙する。
11. 記事へ掲載可能な`仮説と結果の対応`表を作り、Project内部のEvidenceだけでなく読者がアクセスできるEvidence候補も示す。
...
- 事前基準を都合よく変更していない。
- 証拠のない因果関係を断定していない。
- すべての仮説IDが実測、判定、Evidenceへ対応している。
- 数値比較があるのにグラフを作らない場合、その理由と代替表を記録している。
