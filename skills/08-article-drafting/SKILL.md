...
7. `技術選定理由`で次を固定ラベルの実内容として書く: `解決したい課題 / 採用した構成 / 候補・採否・選定理由・不採用理由 / 適用条件 / 非適用条件`。仕組み図を選定理由の代替にしない。
...
14. 実際に発生した失敗は`失敗したこと・TIPS`へ、`発生条件 / 失敗した操作 / エラー全文または主要行 / 原因 / 切り分け / 効果がなかった方法 / 修正内容 / 再実行 / 再実行結果`の固定ラベルで残す。操作、エラー、修正、再実行、結果には実際のコードブロックまたはログを置き、要約文だけで完了しない。
