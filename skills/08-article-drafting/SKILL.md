...
7. `技術選定理由`で次を固定ラベルの実内容として書く: `解決したい課題 / 採用した構成 / この構成を選んだ理由 / 適用条件・制約`。採用理由は最適性の断定に限らず、「複数の可能性から今回はこの仮説を優先して試す」という検証上の決断でもよい。仕組み図を採用理由の代替にしない。
8. 採用理由をResearch Question、実験制約、実測または一次情報へ結ぶ。比較候補や不採用案は、実際に比較した場合だけ任意で記載する。確認していない候補の性能や優劣を捏造しない。
9. 仮説と検証計画を簡潔に説明する。
10. 環境構築を再現可能な手順で書く。
...
14. 実際に発生した失敗は`失敗したこと・TIPS`へ、`発生条件 / 失敗した操作 / エラー全文または主要行 / 原因 / 切り分け / 効果がなかった方法 / 修正内容 / 再実行 / 再実行結果`の固定ラベルで残す。エラーメッセージを最重要Evidenceとする。操作、エラー、修正、再実行、結果には実際のコードブロックまたはログを置き、GUI障害では該当スクリーンショットまたは公開ログへのMarkdownリンクを置く。要約文だけで完了しない。
