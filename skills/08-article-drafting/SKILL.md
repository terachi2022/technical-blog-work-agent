...
7. `技術選定理由`で次を固定ラベルの実内容として書く: `解決したい課題 / 採用した構成 / 候補・採否・選定理由・不採用理由 / 適用条件 / 非適用条件`。仕組み図を選定理由の代替にしない。合理的な代替案が本当にない場合だけ、理由と代替Evidence付きNOT_APPLICABLEを使う。
8. 選定理由をResearch Question、実験制約、実測または一次情報へ結ぶ。確認していない代替案の性能や優劣を捏造せず、未比較なら未比較と明記する。
9. 仮説と検証計画を簡潔に説明する。
10. 環境構築を再現可能な手順で書く。
11. 主要な実行単位を`目的 / 実行 / 観測結果 / 判断`で書く。コード全文は公開Repositoryへ置き、記事では理解に必要な部分へ絞ってよい。
12. `仮説と結果の対応`表で仮説ID、反証条件、実測、判定、Evidenceを結ぶ。
13. 結果を事実として示し、分析と考察を分ける。
14. 実際に発生した失敗は`失敗したこと・TIPS`へ、`発生条件 / 失敗した操作 / エラー全文または主要行 / 原因 / 切り分け / 効果がなかった方法 / 修正内容 / 再実行 / 再実行結果`の固定ラベルで残す。操作、エラー、修正、再実行、結果には実際のコードブロックまたはログを置き、GUI操作では該当スクリーンショットまたは公開ログへのMarkdownリンクを置く。要約文だけで完了しない。
15. 失敗が実際になかった場合だけ、`判定: NOT_APPLICABLE / 理由 / 代替Evidence`を記録する。記録不足をNOT_APPLICABLEにしない。
16. 比較可能な数値がある場合は表またはグラフを使い、比較条件と限界を説明する。
17. `再現用成果物`にRepository、Notebook、raw data、完全ログへの読者向け導線を置く。公開不能なら理由と代替を示す。
18. 追加検証の結果、または実施しない理由と残る不確実性を書く。
19. 制約と未確認事項、参考資料をまとめる。
20. スクリーンショットが未取得なら明確なプレースホルダーを置く。
21. `article-quality-contract.md`を実行し、`results/article-evidence-map.json`を保存する。
22. Agent Repositoryのactivate済みuv環境から次を実行し、決定的検査結果をProjectへ保存する: `python -m evals.check_article_contract --article <Project>/article.md --strict --output <Project>/results/article-contract.json`。
23. commandがexit 0かつReader-visible GateがPASSした場合だけ`quality-review`へ渡す。exit 1なら不足項目を記事修正または上流Evidenceへrouteする。
...
- 技術選定理由が課題、採用案、最低1つの不採用案、適用・非適用条件へ対応している。
